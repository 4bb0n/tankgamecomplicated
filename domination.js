...
class Tank {
  ...
  display() {
    ...
    ctx.fillRect(this.diameter - 5, -10, 30, 20);
...
class Bullet {
  constructor(x, y, angle, owner, speed, bulletWidth) {
    this.x = x;
    this.y = y;
    this.angle = angle;
    this.speed = speed;
    this.radius = bulletWidth / 2;
    this.id = owner;
    this.diameter = bulletWidth;
    this.colour = "#1db4de";
    this.penetration = 0;
    this.damage = 0;
    this.startX = x;
...
class Boss {
  constructor(
    ...
    if(this.type == "gunner"){
        for(let i = 0; i < 3; i++){
        this.nozzles.push(new Nozzle(this.x, this.y, 30, 10, this.angle));
        }
    }
  }
  updateAngle(mouseX, mouseY) {
  ...
  display() {
    ...
    for(let i = 0; i < this.nozzles.length; i++){
        this.nozzles[i].x = this.diameter - 5;
        this.nozzles[i].y = -10 + 10 * i;
        this.nozzles[i].angle = this.angle;
        this.nozzles[i].display()
    }
...
class Dominator {
  ...
  display() {
    ...
    // Draw the nozzle correctly relative to the boss's center
    ctx.fillStyle = "lightgray";
    ctx.fillRect(this.diameter - 5, -10, 30, 40); // Adjusted position
...
let movementSpeed = 2;
let bulletSpeed = 3;
let dominatorBulletSpeed = 10;
let dominatorMovementSpeed = 1;
let bossBulletSpeed = 7;
let enemies = [];
let mazeBlocks = [];
let immune = true;
let maxHealth = 140;
...
socket.on("bulletInfo", (data) => {
  ...
  data.forEach((bullet) => {
    ...
    console.log(receivedBullets[0].x);
  });
});
/*
socket.on("returnEnemies", (data) => {
...
function dominatorShootBullet(){
  dominators.forEach((dominator) => {
    ...
    dominatorBullets.push(
      new Bullet(bulletX, bulletY, dominator.angle, socket.id, dominator.bulletSpeed, 20)
    );
  });
}

function bossShootBullet() { // I used GEMINI 2.5 Pro to help me do the bullet alignment and size depending on the nozzles.
    bosses.forEach((boss) => {
        // Check if the boss has the nozzle array (for types like 'gunner')
        if (boss.nozzles && boss.nozzles.length > 0) {
            // --- Fire from each explicit nozzle ---
            boss.nozzles.forEach((nozzle) => {
                // Get nozzle properties (relative to the rotated boss center)
                const nx = nozzle.x;        // Local x where nozzle rectangle starts
                const ny = nozzle.y;        // Local y where nozzle rectangle starts
                const nw = nozzle.width;    // Nozzle length (drawn width)
                const nh = nozzle.height;   // Nozzle thickness (drawn height)

                // Calculate the LOCAL coordinates of the center of the nozzle's tip
                // Tip is at the end (nx + nw) along the nozzle's drawn width.
                // Center is halfway along its drawn height (ny + nh / 2).
                const localTipX = nx + nw;
                const localTipY = ny + nh / 2;

                // Convert these LOCAL tip coordinates to WORLD coordinates
                const angle = boss.angle; // Angle the boss is facing
                const cosAngle = Math.cos(angle);
                const sinAngle = Math.sin(angle);

                // Rotation formula applied to local tip coordinates, then add boss world center
                const worldTipX = boss.x + localTipX * cosAngle - localTipY * sinAngle;
                const worldTipY = boss.y + localTipX * sinAngle + localTipY * cosAngle;

                // Determine bullet width (e.g., based on nozzle height, or a fixed value)
                const bossBulletWidth = nh * 0.8; // Example: 80% of nozzle thickness

                // Create the bullet at the calculated world position
                // Use boss.id as owner since the boss itself is firing
                bossBullets.push(
                    new Bullet(worldTipX, worldTipY, angle, boss.id, boss.bulletSpeed, bossBulletWidth)
                );
            });
        } else {
             // --- Fallback: Fire from bosses WITHOUT explicit nozzles ---
             // Use a calculation similar to the Dominator/Player if needed
             // Example: Standard single nozzle firing from the edge
             const nozzleBaseOffset = boss.diameter - 5; // Assuming similar drawing convention if not specified
             const nozzleDrawLength = 30;
             const nozzleTipDistance = nozzleBaseOffset + nozzleDrawLength;
             const bossBulletWidth = 15; // Default width

             const worldBulletX = boss.x + Math.cos(boss.angle) * nozzleTipDistance;
             const worldBulletY = boss.y + Math.sin(boss.angle) * nozzleTipDistance;

             bossBullets.push(
                 new Bullet(worldBulletX, worldBulletY, boss.angle, boss.id, boss.bulletSpeed, bossBulletWidth)
             );
        }
    });
}
...
function shootBullet() {
  ...
  bulletsForEnemies.push(
    new Bullet(bulletX, bulletY, tank.angle, socket.id, bulletSpeed, 20)
  );
  ...
  const bulletY2 =
    ...
    Math.sin(tank.angle) * (tank.diameter / 2 + nozzleLength);
  bullets.push(
    new Bullet(bulletX2, bulletY2, tank.angle, socket.id, bulletSpeed, 20)
  );
}
...
function tick() {
  ...
  dominatorBullets.forEach((bullet, index) => {
    ...
    bullets.forEach((bullet2) => {
        ...
        H = Math.sqrt(A * A + O * O);
        if (H < bullet2.diameter + bullet.radius && bullet2.penetration === bullet.penetration) {
            dominatorBullets.splice(index, 1);
            bullets.splice(bullets.indexOf(bullet2), 1);
        }
        else if(H < bullet2.diameter + bullet.radius && bullet2.penetration > bullet.penetration){
            dominatorBullets.splice(index, 1);
            bullet.damage -= dominatorDamage;
        }
        else if(H < bullet2.diameter + bullet.radius && bullet2.penetration < bullet.penetration){
            dominatorBullets.splice(index, 1);
            bullet2.penetration -= bullet.penetration;
        }
